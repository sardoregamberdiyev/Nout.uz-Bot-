...
def get_product(ctg):
    sql = f"""
    SELECT Praduct.id, Praduct.name, Praduct.image, Praduct.narxi, Category.nomi, Category.slug
    FROM Praduct
    inner JOIN Category on Praduct.ctg_id = Category.id
    WHERE Category.nomi = "{ctg}"
    """
    cur.execute(sql)
    return cur.fetchall()
